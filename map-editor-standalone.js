...
function isGroundPaintValue(value) {
  return groundPaintLayers(value) !== null;
}

function editorOperationKey(operation) {
...
function validateEditorEntity(kind, value, { requireIdentity = true } = {}) {
  const errors = [];
  const warnings = [];
...
function validateMapEditorData(value) {
  const errors = [];
...
const EDITOR_MODE_ORDER = Object.freeze(["objects", "terrain", "ground", "npcs", "entrances", "events"]);

function resolveEditorShortcut({ key = "", mode = "objects", modifier = false, shift = false, alt = false } = {}) {
...
function chunkOperationBatches(operations, { id, label = "Cambio", baseRevision = 0, createdAt = Date.now(), maximum = MAP_EDITOR_RULES.limits.operationsPerBatch } = {}) {
...
function resolveConflictQueue(batches, rejectedBatch, { action = "server", revision = 0, retryId = `retry-${Date.now()}` } = {}) {
...
function boundedBrushCells({ col, row, size = 1, cols = MAP_EDITOR_RULES.world.cols, rows = MAP_EDITOR_RULES.world.rows }) {
...
function groundPathType(surface) {
...
function mergeGroundPaintValue(current, selected) {
...
function groundPathConnectionMask({ col, row, getValue, bounds = MAP_EDITOR_RULES.world }) {
...
function lineCells(start, end, bounds = MAP_EDITOR_RULES.world) {
...
function rectangleCells(start, end, bounds = MAP_EDITOR_RULES.world) {
...
function floodFillCells({ start, getValue, bounds = MAP_EDITOR_RULES.world, maximum = MAP_EDITOR_RULES.limits.tileOverrides }) {
...
function changedKeysSince(remoteVersions, revision = 0) {
