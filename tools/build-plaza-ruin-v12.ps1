param(
  [string]$SourceRoot = "assets/generated/plaza-farmacia-pixellab/originals/ruin-v12",
  [string]$LegacySourceRoot = "assets/generated/plaza-farmacia-pixellab/originals/ruin-v11",
  [string]$OutputPath = "assets/generated/plaza-farmacia-pixellab/runtime-v12/buildings/abandoned-megamall-v12.png"
)
...
function New-TransparentPixelLabBitmap {
  ...
  # PixelLab downloads can contain a flat preview matte. Only the
  # edge-connected matte is removed; every architectural pixel stays PixelLab.
...
$legacyRootPath = Resolve-ProjectPath $LegacySourceRoot
$outputPathResolved = Resolve-ProjectPath $OutputPath

$components = [ordered]@{
  Deck = Join-Path $sourceRootPath "upper-ruined-deck-a761b689.png"
  West = Join-Path $legacyRootPath "west-wing-83b8beb7.png"
  Atrium = Join-Path $legacyRootPath "central-atrium-7b4a272f.png"
  East = Join-Path $legacyRootPath "east-wing-95d906d0.png"
}
...
try {
  ...
  # The new PixelLab deck establishes the frontal-superior depth first.
  # The existing PixelLab facade masses then overlap its south edge, creating
  # one continuous megamall with an integrated central atrium/entrance.
  Draw-ObjectInBox $graphics $components.Deck ([System.Drawing.Rectangle]::new(16, 0, 1120, 372))
  Draw-ObjectInBox $graphics $components.West ([System.Drawing.Rectangle]::new(-24, 254, 548, 254))
  Draw-ObjectInBox $graphics $components.East ([System.Drawing.Rectangle]::new(628, 254, 548, 254))
  Draw-ObjectInBox $graphics $components.Atrium ([System.Drawing.Rectangle]::new(336, 226, 480, 284))
} finally {
  $graphics.Dispose()
}
...
try {
  ...
  if ($opaqueWidth -lt 1080 -or $opaqueHeight -lt 440) {
